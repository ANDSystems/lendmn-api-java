    ...
    try {
      ...
      if (code != 0) {
        JsonNode message = root.path("response")
            .path("error_message");
        JsonNode key = root.path("response")
            .path("error");
        ApiError apiError = ApiError.get(code);
        if(message.isMissingNode()) {
          message = root.path("response")
              .path("error_description");
          if(message.isMissingNode() && key.isMissingNode()) {
            throw new ApiException(apiError);
          } else if(message.isMissingNode() && !key.isMissingNode()) {
            throw new ApiException(apiError, null, key.asText());
          }
        } 
        if(key.isMissingNode()) {
          ...
          throw new ApiException(apiError, message.asText(), key.asText());
        }
      }
